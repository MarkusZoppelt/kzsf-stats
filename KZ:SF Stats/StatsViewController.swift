...
class StatsViewController: UIViewController
{
    ...
    let clanLabel = UILabel(frame: CGRectMake(UIScreen.mainScreen().bounds.width/2+10, 0*165-40+70, UIScreen.mainScreen().bounds.width/2-30, 50))
    ...
    let captureLabel = UILabel(frame: CGRectMake(25, 4*165-40 + 40, UIScreen.mainScreen().bounds.width/2-30, 50))
    let beaconLabel = UILabel(frame: CGRectMake(25, 4*165-40 + 70, UIScreen.mainScreen().bounds.width/2-30, 50))
    let echoLabel = UILabel(frame: CGRectMake(25, 4*165-40 + 100, UIScreen.mainScreen().bounds.width/2-30, 50))
    let chargesLabel = UILabel(frame: CGRectMake(25, 4*165-40 + 130, UIScreen.mainScreen().bounds.width/2-30, 50))
    let teamkillsLabel = UILabel(frame: CGRectMake(25, 4*165-40 + 160, UIScreen.mainScreen().bounds.width/2-30, 50))
...
//                var j = CGFloat(45+i)
        ...
        clanLabel.textColor = UIColor.yellowColor()
        clanLabel.textAlignment = NSTextAlignment.Right
        clanLabel.font = UIFont(name: "Avenir Next Condensed", size: 20.0)
        
        clanButton.frame = CGRectMake(UIScreen.mainScreen().bounds.width/2+10, 0*165-40+70, UIScreen.mainScreen().bounds.width/2-30, 50)
        clanButton.backgroundColor = UIColor.clearColor()
        clanButton.addTarget(self, action: "showClanInfoViaButton:", forControlEvents: UIControlEvents.TouchUpInside)
    ...
    func showClanInfoViaButton(sender: UIButton)
    {
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        if(firstLoad)
        {
            let infoWindow = UIAlertController(title: "Navigation", message: "Swipe left/right to view clan/user stats", preferredStyle: UIAlertControllerStyle.Alert)
            infoWindow.addAction(UIAlertAction(title: "OK", style: UIAlertActionStyle.Default, handler: { (action:UIAlertAction!) -> Void in
                self.dismissViewControllerAnimated(true, completion: nil)
            }))
            //        infoWindow.view.tintColor = UIColor.cyanColor()
            //        infoWindow.view.backgroundColor = UIColor.blackColor()
            self.presentViewController(infoWindow, animated: true, completion: nil)
            firstLoad = false
        }
    }
...
//        var user = "Mirokke"
        ...
        else
        {
            ...
            var clanTag = clanStats["tag"].asString
            if  clanTag == nil
            {
                clanTag = "-"
            }
